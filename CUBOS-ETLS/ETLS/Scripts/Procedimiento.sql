        ...
        BEGIN TRY
            ...
            SELECT
               ...
               -- DATEADD(DAY, ABS(CHECKSUM(NEWID()) % 365), CONVERT(DATE, GETDATE())) AS Flight_Date,
				DATEADD(DAY,ABS(CHECKSUM(NEWID()) % 365), DATEADD(YEAR, ABS(CHECKSUM(NEWID()) % 5) * -1, CONVERT(DATE, GETDATE()))) AS Flight_Date,
    ...
    BEGIN TRY
        ...
        SELECT
		 -- Fecha aleatoria en el futuro o el día actual
			DATEADD(DAY,ABS(CHECKSUM(NEWID()) % 365), DATEADD(YEAR, ABS(CHECKSUM(NEWID()) % 5) * -1, CONVERT(DATE, GETDATE()))) AS Date,
...
BEGIN
    BEGIN TRY
        ...
        SELECT 
			...
			DATEADD(DAY,ABS(CHECKSUM(NEWID()) % 365), DATEADD(YEAR, ABS(CHECKSUM(NEWID()) % 5) * -1, CONVERT(DATE, GETDATE()))) AS Date_Ticket,
    ...
    BEGIN
        BEGIN TRY
            ...
            SELECT
                ...
                DATEADD(DAY,ABS(CHECKSUM(NEWID()) % 365), DATEADD(YEAR, ABS(CHECKSUM(NEWID()) % 5) * -1, CONVERT(DATE, GETDATE()))) AS Reservation_Date,
